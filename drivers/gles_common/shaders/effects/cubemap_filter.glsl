...
void main() {
	...
	gl_FragColor = vec4(textureCubeLod(source_cube, N, 0.0).rgb, 1.0);
#else
	...
	// MAX_SAMPLE_COUNT is injected into the shader during compilation.
	for (int sample_num = 0; sample_num < MAX_SAMPLE_COUNT; sample_num++) {
		...
		vec3 val = textureCubeLod(source_cube, L, sample_direction_mip.w).rgb;
		// Mix using linear
