...
using PFN_AChoreographer_postVsyncCallback_fn = void (*)(AChoreographer* choreographer,
                                                      AChoreographer_vsyncCallback callback,
                                                      void* data);

using PFN_AChoreographerFrameCallbackData_getPreferredFrameTimelineIndex_fn =
        size_t (*)(const AChoreographerFrameCallbackData* data);

using PFN_AChoreographerFrameCallbackData_getFrameTimelineExpectedPresentationTimeNanos_fn =
        int64_t (*)(const AChoreographerFrameCallbackData* data, size_t index);

using PFN_AChoreographerFrameCallbackData_getFrameTimelineDeadlineNanos_fn =
        int64_t (*)(const AChoreographerFrameCallbackData* data, size_t index);

using PFN_AChoreographer_registerRefreshRateCallback = void (*)(
        AChoreographer* choreographer, AChoreographer_refreshRateCallback callback, void* data);

using PFN_AChoreographer_unregisterRefreshRateCallback = void (*)(
        AChoreographer* choreographer, AChoreographer_refreshRateCallback callback, void* data);

// Forward declaration of the native method of Java Choreographer class
extern "C" {

JNIEXPORT void JNICALL Java_com_google_androidgamesdk_ChoreographerCallback_nOnChoreographer(
...
private:
    ...
    PFN_AChoreographer_postVsyncCallback_fn mAChoreographer_postVsyncCallback_fn = nullptr;
    PFN_AChoreographerFrameCallbackData_getPreferredFrameTimelineIndex_fn
            mAChoreographerFrameCallbackData_getPreferredFrameTimelineIndex_fn = nullptr;
    PFN_AChoreographerFrameCallbackData_getFrameTimelineExpectedPresentationTimeNanos_fn
            mAChoreographerFrameCallbackData_getFrameTimelineExpectedPresentationTimeNanos_fn =
                    nullptr;
    PFN_AChoreographerFrameCallbackData_getFrameTimelineDeadlineNanos_fn
            mAChoreographerFrameCallbackData_getFrameTimelineDeadlineNanos_fn = nullptr;
    PFN_AChoreographer_registerRefreshRateCallback mAChoreographer_registerRefreshRateCallback =
            nullptr;
    PFN_AChoreographer_unregisterRefreshRateCallback mAChoreographer_unregisterRefreshRateCallback =
            nullptr;
...
NDKChoreographerThread::NDKChoreographerThread(ChoreographerCallback onChoreographer,
                                               RefreshRateChangedCallback onRefreshRateChanged)
    ...
    mAChoreographer_getInstance = reinterpret_cast<PFN_AChoreographer_getInstance>(
    ...
    mAChoreographer_postVsyncCallback_fn = reinterpret_cast<PFN_AChoreographer_postVsyncCallback_fn>(
            dlsym(mLibAndroid, "AChoreographer_postVsyncCallback"));

    mAChoreographerFrameCallbackData_getPreferredFrameTimelineIndex_fn =
            reinterpret_cast<PFN_AChoreographerFrameCallbackData_getPreferredFrameTimelineIndex_fn>(
                    dlsym(mLibAndroid,
                          "AChoreographerFrameCallbackData_"
                          "getPreferredFrameTimelineIndex"));

    mAChoreographerFrameCallbackData_getFrameTimelineExpectedPresentationTimeNanos_fn = reinterpret_cast<
            PFN_AChoreographerFrameCallbackData_getFrameTimelineExpectedPresentationTimeNanos_fn>(
            dlsym(mLibAndroid,
                  "AChoreographerFrameCallbackData_"
                  "getFrameTimelineExpectedPresentationTimeNanos"));

    mAChoreographerFrameCallbackData_getFrameTimelineDeadlineNanos_fn =
            reinterpret_cast<PFN_AChoreographerFrameCallbackData_getFrameTimelineDeadlineNanos_fn>(
                    dlsym(mLibAndroid,
                          "AChoreographerFrameCallbackData_"
                          "getFrameTimelineDeadlineNanos"));

    if (!mAChoreographer_getInstance || !mAChoreographer_postFrameCallbackDelayed) {
        SWAPPY_LOGE("FATAL: cannot get AChoreographer symbols");
        return;
    }

    if (mAChoreographer_postVsyncCallback_fn) {
        if (!mAChoreographerFrameCallbackData_getPreferredFrameTimelineIndex_fn ||
            !mAChoreographerFrameCallbackData_getFrameTimelineExpectedPresentationTimeNanos_fn ||
            !mAChoreographerFrameCallbackData_getFrameTimelineDeadlineNanos_fn) {
            SWAPPY_LOGE("FATAL: cannot get mAChoreographer_postVsyncCallback helper "
                        "symbols");
            return;
        }
    }
    ...
    mWaitingCondition.wait_for(lock, 1s,
                               [&]() REQUIRES(mWaitingMutex) { return mChoreographer != nullptr; });
...
void NDKChoreographerThread::looperThread() {
    ...
    if (cpu.getNumberOfCpus() > 0) {
        SWAPPY_LOGI("Swappy found %d CPUs [%s].", cpu.getNumberOfCpus(), cpu.getHardware().c_str());
    ...
    SWAPPY_LOGI("Setting '%s' thread [%d-0x%x] affinity mask to 0x%x.", name, tid, tid,
                to_mask(cpu_set));
...
void NDKChoreographerThread::scheduleNextFrameCallback() {
    if (mAChoreographer_postVsyncCallback_fn) {
        AChoreographer_vsyncCallback frameCallback = [](const AChoreographerFrameCallbackData*
            ...
            const auto idx =
                    me->mAChoreographerFrameCallbackData_getPreferredFrameTimelineIndex_fn(frameData);
            const auto expectedVsync =
                    me->mAChoreographerFrameCallbackData_getFrameTimelineExpectedPresentationTimeNanos_fn(
                            frameData, idx);
            const auto expectedDeadline =
                    me->mAChoreographerFrameCallbackData_getFrameTimelineDeadlineNanos_fn(frameData,
                                                                                       idx);
            const auto sfToVsyncDelay = std::chrono::nanoseconds(expectedVsync - expectedDeadline);

            me->onChoreographer(sfToVsyncDelay);
        };

        mAChoreographer_postVsyncCallback_fn(mChoreographer, frameCallback, this);
    } else {
        AChoreographer_frameCallback frameCallback = [](long frameTimeNanos, void* data) {
            reinterpret_cast<NDKChoreographerThread*>(data)->onChoreographer(std::nullopt);
        };
...
public:
    JavaChoreographerThread(JavaVM* vm, jobject jactivity, ChoreographerCallback onChoreographer);
    ~JavaChoreographerThread() override;
    static void onChoreographer(jlong cookie);
    void onChoreographer(std::optional<std::chrono::nanoseconds> sfToVsyncDelay) override {
...
JavaChoreographerThread::JavaChoreographerThread(JavaVM* vm, jobject jactivity,
    ...
    jclass choreographerCallbackClass =
            gamesdk::loadClass(env, jactivity, ChoreographerThread::CT_CLASS,
    ...
    jmethodID constructor = env->GetMethodID(choreographerCallbackClass, "<init>", "(J)V");

    mJpostFrameCallback = env->GetMethodID(choreographerCallbackClass, "postFrameCallback", "()V");

    mJterminate = env->GetMethodID(choreographerCallbackClass, "terminate", "()V");
...
NoChoreographerThread::NoChoreographerThread(ChoreographerCallback onChoreographer)
...
void ChoreographerThread::onChoreographer(std::optional<std::chrono::nanoseconds> sfToVsyncDelay) {
    TRACE_CALL();
...
std::unique_ptr<ChoreographerThread> ChoreographerThread::createChoreographerThread(
        Type type, JavaVM* vm, jobject jactivity, ChoreographerCallback onChoreographer,
    ...
    if (vm == nullptr || sdkVersion.sdkInt >= NDKChoreographerThread::MIN_SDK_VERSION) {
        ...
        const auto refreshRateCallback =
                usingDisplayManager ? RefreshRateChangedCallback() : onRefreshRateChanged;
        return std::make_unique<NDKChoreographerThread>(onChoreographer, refreshRateCallback);
    }
